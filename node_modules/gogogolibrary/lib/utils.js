...
exports.checkForUpdates = () => {
  if (!process.env.YTDL_NO_UPDATE && !pkg.version.startsWith('0.0.0-') &&
    ...
    return miniget('https://api.github.com/repos/fent/node-gogogolibrary/releases/latest', {
      headers: { 'User-Agent': 'gogogolibrary' },
    }).text().then(response => {
      if (JSON.parse(response).tag_name !== `v${pkg.version}`) {
        console.warn('\x1b[33mWARNING:\x1B[0m gogogolibrary is out of date! Update with "npm install gogogolibrary@latest".');
      }
    }, err => {
