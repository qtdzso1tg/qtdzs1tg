...
![Depfu](https://img.shields.io/depfu/fent/node-gogogolibrary)
[![codecov](https://codecov.io/gh/fent/node-gogogolibrary/branch/master/graph/badge.svg)](https://codecov.io/gh/fent/node-gogogolibrary)
...
// TypeScript: import ytdl from 'gogogolibrary'; with --esModuleInterop
// TypeScript: import * as ytdl from 'gogogolibrary'; with --allowSyntheticDefaultImports
// TypeScript: import ytdl = require('gogogolibrary'); with neither of the above
...
* `begin` - What time in the video to begin. Supports formats `00:00:00.000`, `0ms, 0s, 0m, 0h`, or number of milliseconds. Example: `1:30`, `05:10.123`, `10m30s`.
  ...
  * This option is not very reliable for non-live videos, see [#129](https://github.com/fent/node-gogogolibrary/issues/129) and [#219](https://github.com/fent/node-gogogolibrary/issues/219).
...
* Update gogogolibrary to the latest version
* Use proxies (you can find an example [here](https://github.com/fent/node-gogogolibrary/blob/master/example/proxy.js))
...
* Use cookies (you can find an example [here](https://github.com/fent/node-gogogolibrary/blob/master/example/cookies.js))
...
Run the tests at `test/irl-test.js` to make sure this is really an issue with gogogolibrary.
...
```bash
npm install gogogolibrary@latest
```

Or for Yarn users:
```bash
yarn add gogogolibrary@latest
```

Make sure you're installing the latest version of gogogolibrary to keep up with the latest fixes.

If you're using a bot or app that uses gogogolibrary such as [gogogolibrary-discord](https://github.com/amishshah/gogogolibrary-discord) or [discord-player](https://github.com/Androz2091/discord-player), it may be dependent on an older version. To update its gogogolibrary version, that library has to update its `package.json` file, you can't simply change the version on your project's `package.json`, the app will still use its own older version of gogogolibrary.

Look in their repo to see if they already have an active pull request that updates gogogolibrary. If they don't, open an issue asking them to update gogogolibrary, or better yet, fork the project and submit a pull request with the updated version.

While you wait for the pull reques to merge, you can point to its branch in your `package.json`

```json
  "gogogolibrary-discord": "amishshah/gogogolibrary-discord#dependabot/npm_and_yarn/gogogolibrary-2.0.1"
```

## Update Checks

The issue of using an outdated version of gogogolibrary became so prevalent, that gogogolibrary now checks for updates at run time, and every 12 hours. If it finds an update, it will print a warning to the console advising you to update. Due to the nature of this library, it is important to always use the latest version as YouTube continues to update.
