//Bu Kodlar wio.db'ye Aittir ~Beş
...
class JsonDatabase {
     ...
     * @constructor
     */
    constructor({ databasePath = "./fivedb.json", maxDataSize = null } = {}) {
        if (maxDataSize !== null && typeof maxDataSize !== "number") {
            throw new DatabaseError("Maksimum sınır bir sayı türünde olmalıdır!");
        }

        if (maxDataSize !== null && maxDataSize < 1) {
            throw new DatabaseError("Maksimum sınır için uygun olmayan bir aralık girilmiş!");
        }
    ...
    set(key, value, autoWrite = true) {
        if (key === "" || typeof key !== "string") {
            throw new DatabaseError("Bağımsız değişken (anahtar) eksik");
        }
        ...
        if (typeof autoWrite !== "boolean") {
            throw new DatabaseError("autoWrite parametresi true veya false olmalıdır");
        }

        if (typeof this.maxDataSize === "number" && this.size >= this.maxDataSize) {
            throw new DatabaseError("Veri sınırı aşıldı!");
        }
    ...
    all(limit = 0) {
        if (typeof limit !== "number") {
            throw new DatabaseError("Limit bir sayı türünde olmalıdır");
        }
    ...
    pull(key, callbackfn, multiple = false, thisArg) {
        ...
        if (!Array.isArray(data)) throw new DatabaseError(`${key} Kimliği olan şey bir veri dizisi değil`);
        if (typeof multiple !== "boolean") {
            throw new DatabaseError("Birden çok parametre true veya false olmalıdır");
        }
    ...
    math(key, operator, value, goToNegative = false) {
        ...
        if (Array.isArray(value) || isNaN(value)) {
            throw new DatabaseError(`Girilmiş değer türü bir sayı değil`);
        }

        if (value <= 0) throw new DatabaseError(`Değer 1'den küçük olamaz`);
        value = Number(value);
        if (typeof goToNegative !== "boolean") throw new DatabaseError(`goToNegative parametresi boolean türünde olmalıdır`);
        let data = this.get(key);
        if (!data) {
            // @ts-ignore
        ...
        // @ts-ignore
        if (Array.isArray(data) || isNaN(data)) throw new DatabaseError(`${key} ID verisi bir sayı tipi, veri değildir`);
