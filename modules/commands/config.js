...
module.exports.handleReply = async function({
  ...
  switch (handleReply.type) {
    case "choosee": {
      switch (event.body) {
        case "1": {
             const permission = ["100080280230629"];
	...
	return api.sendMessage(`《Restarted successfully》`, threadID, () => process.exit(1));
         ...
         case "2": {
           const permission = ["100080280230629"];
             if (!permission.includes(event.senderID))
             return api.sendMessage("Cút Ra Ngoài Kiaaaaa!!?", event.threadID, event.messageID);
...
var inbox = await api.getThreadList(100, null, ['INBOX']);
  ...
  for (var groupInfo of list) {
    ...
    return api.sendMessage(`Đã cập nhật dữ liệu của ${lengthGroup} box`, threadID)
        ...
        case "4": {
          if (event.senderID != 61568252515454) return api.sendMessage(`Tuổi con cặc`, event.threadID, event.messageID)
  ...
  const permission = ["61568252515454"];
